"""Unified client creation for Connect and gRPC protocols."""
...
def create_client_sync(
    service_class: type[Any], transport: Transport | ConnectTransport | GrpcTransport
) -> Any:
    """Create a synchronous client for the given service using the specified transport.

    Args:
        service_class: The service class containing metadata (e.g., Haberdasher)
        transport: The transport to use (Connect or gRPC)

    Returns:
        A client instance appropriate for the transport type

    Example:
        ```python
        # For Connect protocol
        from connecpy.transport import ConnectTransport, create_client_sync
        from example.haberdasher_connecpy import Haberdasher, HaberdasherClientSync

        connect_transport = ConnectTransport("http://localhost:3000")
        client = create_client_sync(Haberdasher, connect_transport)
        # Returns HaberdasherClientSync instance

        # For gRPC protocol
        from connecpy.transport import GrpcTransport, create_client_sync

        grpc_transport = GrpcTransport("localhost:50051")
        client = create_client_sync(Haberdasher, grpc_transport)
        # Returns GrpcClientWrapper wrapping HaberdasherStub
        ```
    """
    if isinstance(transport, ConnectTransport):
        # For Connect transport, return the existing ConnecpyClientSync-based client
        # This assumes the service_class has a reference to the client class
        # In practice, this would be generated by protoc-gen-connecpy
        ...
        client_class_name = f"{base_name}ClientSync"
    ...
    if isinstance(transport, GrpcTransport):
        ...
        wrapper_class_name = f"{base_name}GrpcWrapperSync"
        ...
        # Generic transport with Transport protocol
        # Create a dynamic client that uses the transport methods
        class DynamicClient:
            def __init__(self, transport: Transport, service_info: Any) -> None:
            ...
            def _create_method(self, method_info: MethodInfo) -> Any:
                def method_impl(
                    ...
                    *,
                    ...
                    return self._transport.unary_unary(
        ...
        return DynamicClient(transport, service_info)  # type: ignore[arg-type]
