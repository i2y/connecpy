...
from connecpy.compression import CompressionMiddleware, GenericEncodingMiddleware
...
@pytest.mark.asyncio
async def test_compression_no_encoding():
    app = AsyncMock()
...
@pytest.mark.asyncio
async def test_compression_gzip_encoding():
    app = AsyncMock()
    middleware = CompressionMiddleware(app)
    scope = {
        "type": "http",
    ...
    await middleware(scope, receive, send)
    # Now GZipMiddleware can read the correct header tuple
    app.assert_awaited_once()


@pytest.mark.asyncio
async def test_compression_unknown():
    app = AsyncMock()
    middleware = CompressionMiddleware(app)
    scope = {
        "type": "http",
        "headers": [(b"accept-encoding", b"unknown")],
    }
    receive = Mock()
    send = Mock()
    ...
    try:
        ...
        assert "Unsupported Content-Encoding" in str(e)
